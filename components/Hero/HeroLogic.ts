...
export function useHeroLogic() {
  ...
  // Noul state care ține minte EXACT ce card este în centrul ecranului
  ...
  useEffect(() => {
    ...
    ScrollTrigger.create({
      ...
      onEnter: () => gsap.to('.hero-ui-hide', { opacity: 0, scale: 0.95, duration: 0.2, overwrite: true, pointerEvents: 'none' }),
      onLeaveBack: () => gsap.to('.hero-ui-hide', { opacity: 1, scale: 1, duration: 0.2, overwrite: true, pointerEvents: 'auto' })
    });

    mm.add("(min-width: 768px)", () => {
      const tl = gsap.timeline({ scrollTrigger: { trigger: containerRef.current, start: "top top", end: "+=150%", scrub: 1, pin: true } });

      tl.to("#headerLogo", { scale: 0.75, color: "#ffffff", duration: 0.5, ease: "none" }, 0);
      tl.to("#headerContact", { color: "#ffffff", duration: 0.5, ease: "none" }, 0);
      ...
      tl.to(creamLayerRef.current, { clipPath: "inset(7.5vh 32.5vw 7.5vh 32.5vw round 16px)", duration: 0.6, ease: "power2.inOut" }, 0);
    ...
    mm.add("(max-width: 767px)", () => {
      ...
      tl.to(marqueeContainerRef.current, { opacity: 0.25, duration: 0.4 }, 0);
      tl.to(marquee1Ref.current, { x: "20vw", duration: 1, ease: "none" }, 0);
      tl.to(marquee2Ref.current, { x: "-20vw", duration: 1, ease: "none" }, 0);

      tl.to(creamLayerRef.current, { clipPath: "inset(20vh 17.5vw 20vh 17.5vw round 16px)", duration: 0.6, ease: "power2.inOut" }, 0);
      tl.to(creamLayerRef.current, { opacity: 0, duration: 0.4 }, 0.6);
      tl.to(imageRef.current, { opacity: 1, duration: 0.4 }, 0.6);
      tl.to(grayOverlayRef.current, { opacity: 1, duration: 0.4 }, 0.6);
      tl.to(signatureRef.current, { opacity: 1, scale: 1, duration: 0.4, ease: "power2.out" }, 0.8);
  ...
  return {
    refs: { containerRef, imageRef, creamLayerRef, signatureRef, grayOverlayRef, marqueeContainerRef, marquee1Ref, marquee2Ref },
    state: { isMenuOpen, setIsMenuOpen, activeMap, setActiveMap, isMapOpen, setIsMapOpen, isOrderModalOpen, setIsOrderModalOpen, selectedPrice, setSelectedPrice, activeCard, setActiveCard },
    data: { mapUrls }
  };
}